...
namespace NativeWifi
{
	...
	public static class Wlan
	{
		...
		public struct Dot11Ssid
		{
            ...
            public bool Equals(Dot11Ssid obj)
            {
                int i = 0;
                if (this.SSIDLength != obj.SSIDLength) return false;
                while (i < this.SSIDLength & obj.SSID[i] == this.SSID[i])
                {
                    i++;
                }
                return obj.SSID[i] == this.SSID[i];
            }
